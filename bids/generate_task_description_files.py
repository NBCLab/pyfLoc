...
events_description = {
    ...
    'accuracy': {
        'LongName': 'accuracy',
        'Description': ('Participant performance. Trials are automatically '
    ...
    'classification': {
        ...
        'Levels': {
            'true_positive': ('A correct button-press on a task target trial. '
                              'Also referred to as a "hit".'),
            'false_positive': ('An incorrect button-press on a baseline trial. '
                               'Also referred to as a "false alarm".'),
            'false_negative': ('An incorrect non-press on a task target trial. '
...
for task_type in ['Oddball', 'Two-Back', 'One-Back']:
    ...
    with open('task-localizer{0}_events.json'.format(task_type.replace('-', '')), 'w') as fo:
        json.dump(events_description, fo, sort_keys=True, indent=4)

    with open('task-localizer{0}_bold.json'.format(task_type.replace('-', '')), 'w') as fo:
